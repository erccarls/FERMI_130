...
def Gen_Plots(fileout = 'Cluster_Example_48'):
    ...
    #<!><!><!><!><!><!><!><!><!><!><!><!><!><!><!><!><!><!>
    print 
    sig_null_95= stats.scoreatpercentile(sigs[1], 5) # 2 sigma
    print "There is a 5%% chance to obtain significances below %f in case of pulsars" % sig_null_95
    stat_power = stats.percentileofscore(sigs[0], sig_null_95)
    print "Probability to reject wrong null (pulsar) hypothesis at 95%%CL is %.1f%%" % stat_power
    #<!><!><!><!><!><!><!><!><!><!><!><!><!><!><!><!><!><!>
    #<!><!><!><!><!><!><!><!><!><!><!><!><!><!><!><!><!><!>
    ...
    plt.axes([0.12, 0.14, 0.85, 0.8])
    me, si, sc = zip(members, sigs, scales)[1]
    plt.plot(me, si, ls='', marker='o', label='Pulsars', color='k')
    me, si, sc = zip(members, sigs, scales)[0]
    plt.plot(me, si, ls='', marker='o', label='NFW', color='r')
    plt.legend(frameon=True, handlelength=0.6, loc=4)
    plt.xlim([0, 15])
    plt.xlabel(r'$N_{members}$')
    plt.ylim([0, 4])
    plt.ylabel(r'S')
    plt.savefig('dist_2d_1.pdf')

    plt.clf()
    plt.figure(3, figsize=(5, 4))
    plt.axes([0.12, 0.14, 0.85, 0.8])
    me, si, sc = zip(members, sigs, scales)[1]
    plt.plot(me, sc, ls='', marker='o', label='Pulsars', color='k')
    me, si, sc = zip(members, sigs, scales)[0]
    plt.plot(me, sc, ls='', marker='o', label='NFW', color='r')
    plt.legend(frameon=True, handlelength=0.6, loc=1)
    plt.xlim([0, 15])
    plt.xlabel(r'$N_{members}$')
    plt.ylim([0, 1.0])
    plt.ylabel(r'$r_{cluster}[^\circ]$')
    plt.savefig('dist_2d_2.pdf')

    plt.clf()
    plt.figure(4, figsize=(5, 4))
    plt.axes([0.12, 0.14, 0.85, 0.8])
    me, si, sc = zip(members, sigs, scales)[1]
    plt.plot(si, sc, ls='', marker='o', label='Pulsars', color='k')
    me, si, sc = zip(members, sigs, scales)[0]
    plt.plot(si, sc, ls='', marker='o', label='NFW', color='r')
    plt.legend(frameon=True, handlelength=0.6, loc=1)
    plt.xlim([0, 4])
    plt.xlabel(r'S')
    plt.ylim([0, 1.0])
    plt.ylabel(r'$r_{cluster}[^\circ]$')
    plt.savefig('dist_2d_3.pdf')
