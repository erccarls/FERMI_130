...
def DBSCAN_Compute_Clusters(mcSims, eps = 0.1875,n_cluster = 3,nCore = 3, S_cut=2.0,numAnalyze=0, fileout = '',BGTemplate = 'BGRateMap.pickle', HESS = False, angularSize = 10.,SNR= 0.75,numProcs = 10):
    '''
    ...
    DBSCAN_PARTIAL = partial(DBSCAN_THREAD,  eps=eps, n_cluster=n_cluster,nCore = nCore, plot=False)
...
def DBSCAN_THREAD(sim, eps, n_cluster,nCore , plot=False):
    xVec = sim[0]
    yVec = sim[1]
    X = []
    for i in range(len(xVec)):
        X.append((xVec[i],yVec[i]))
    return (DBSCAN.RunDBScan(X, eps, n_cluster,nCore = nCore, plot=False), len(xVec))
